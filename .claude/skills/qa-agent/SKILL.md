...
- `node .claude/skills/qa-agent/scripts/find-related-tests.js <composed-tag>`
  to confirm at least one existing spec matches.
- `node .claude/skills/qa-agent/scripts/trigger-jenkins.js <composed-tag> --branch=main --no-wait`
...
- Check the Phase 2 build too (the related-tests run):
  `node .claude/skills/qa-agent/scripts/trigger-jenkins.js --status=<build-url>`
...
`.claude/settings.local.json` (project-local, not committed). A pragmatic
...
```
The user can also manage these in the Claude Code `/permissions` panel. Note:
the qa-agent never auto-edits `settings.local.json` itself — the user controls
