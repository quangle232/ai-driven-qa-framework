...
- **Mirror parity**: `.agents/skills` and `.agents/skills` have the same skills + content
...
reference or a skill, **mirror the change to BOTH `.claude` and `.agents`** (fix `.agents/`→`.agents/`
